...
## 1. Введение
- В параллельном программировании важны связи между процессами для обмена и распределения данных. Для организации структуры узлов и линий в сети используются топологии. Их можно представть в виде графа, где вершины — процессоры (процессы), а дуги — каналы связи.
...
**Входные данные:**
- Заголовок сообщения (структура MessageHeader), содержащий:
  - Флаг доставки (delivered).
  - Номер процесса-источника.
  - Номер процесса-получателя.
  - Размер данных.
- Данные (MessageData) — вектор целых чисел произвольной длины N.
- Далее обе структуры вместе будут называться "сообщение".
...
- Сообщение состоит из заголовка (MessageHeader) и данных (MessageData).
- Заголовок передаётся отдельно от данных по-байтово (MPI_BYTE).
...
- Проверка выполняется на процессе-источнике
...
- `SendMessageMPI` — отправка сообщения с заголовком и данными.
- `RecvMessageMPI` — приём сообщения.
...
| ----- | ---------------- | ------------------ | --------------- | -------- | --------- | ------------- |
| mpi   | 0                | 1                  | 2               | 0.294714 | 1.0000    | N/A           |
| mpi   | 0                | 3                  | 4               | 0.427870 | 0.5420    | 13.6%         |
| mpi   | 0                | 5                  | 6               | 0.427870 | 0.5420    | 13.6%         |

**WSL:**
| Режим | Процесс-источник | Процесс-получатель | Число процессов | Время, s | Ускорение | Эффективность |
| ----- | ---------------- | ------------------ | --------------- | -------- | --------- | ------------- |
| mpi   | 0                | 1                  | 2               | 1.634773 | 1.0000    | N/A           |
| mpi   | 0                | 3                  | 4               | 2.616761 | 0.6247    | 31.2%         |
| mpi   | 0                | 5                  | 6               | 3.890610 | 0.4202    | 14.0%         |

**\*GitHub:**
| Режим | Процесс-источник | Процесс-получатель | Число процессов | Время, s | Ускорение | Эффективность |
| ----- | ---------------- | ------------------ | --------------- | -------- | --------- | ------------- |
| mpi   | 0                | 1                  | 2               | 0.664711 | 1.0000    | N/A           |
...
- MPI версия:
  ...
  - Эффективно использует коллективные операции.
...
- Ограничение масштабируемости:
  ...
  - Большие объемы данных требут значительных буферов и времени копирования между процессами.
